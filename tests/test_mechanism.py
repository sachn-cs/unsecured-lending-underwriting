...
def make_svc() -> Handler:
    return Handler(name="test-mech", bus=LocalBus(), store=MemoryStore())
...
class TestAddSeed:
    ...
    def test_emits_seed_added_event(self) -> None:
        ...
        svc = MechHandler(name="mech", bus=bus, store=MemoryStore())
        svc.start()
        bus.start()
        command(svc, "add_seed", {"user": "bank", "base_budget": 100_000})
...
class TestMechanismStateOrdering:
    def test_add_seed_emits_after_persist(self) -> None:
        ...
        svc = Handler(name="test-mech", bus=bus, store=MemoryStore())
        svc.start()
        command(svc, "add_seed", {"user": "bank", "base_budget": 100_000})
